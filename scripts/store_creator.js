...
function addToStorage(itemID, sizeValue, optionValue, priceList, quantity){
            ...
            if (existingItemString) {
                ...
                if (existingItemData[0] === newItemData[0] &&
                    ...
                    if (existingItemData[4] === newItemData[4]) {
                        quantityChange = true; //Raising flag if everything is exactly the same
                    } else {
    ...
    let existingItemString = localStorage.getItem(targetKey);
    let existingItemData = JSON.parse(existingItemString);

    //Warns user and does no operation
    if (isDuplicate) {
        alert("This error is supposed to be not posible!\nHow did you get it?");
    }
    //Changes the quntity of the item, removing the popUp afterwards
    else if (quantityChange) {
        quantityItemData = [newItemData[0], newItemData[1], newItemData[2], newItemData[3], Number(newItemData[4]) + Number(existingItemData[4])];

        localStorage.setItem(targetKey, JSON.stringify(quantityItemData));
        alert("Quantity of an existing item has been updated!\nQuantity of " + newItemData[0] + " Changed from " + existingItemData[4] + " to " + quantityItemData[4]);
...
function addToStorageShirt(itemID, sizeValue, optionValue, priceList, quantity, gender){
    ...
    if (isDuplicate) {
        ...
        quantityItemData = [newItemData[0], newItemData[1], newItemData[2], newItemData[3], Number(newItemData[4]) + Number(existingItemData[4]), newItemData[5]];
